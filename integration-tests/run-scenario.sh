...
if [ "${os}" = "Windows" ]; then
  ...
  sshpass -p "${password}" scp -r -q -o StrictHostKeyChecking=no ${user}@${host}:${REM_DIR}/surefire-reports ${OUTPUTS_DIR}/scenarios/integration-tests
  ...
  scp -o StrictHostKeyChecking=no -r -i ${key_pem} ${user}@${host}:${REM_DIR}/surefire-reports ${OUTPUTS_DIR}/scenarios/integration-tests
